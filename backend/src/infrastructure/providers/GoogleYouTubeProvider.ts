...
export class GoogleYouTubeProvider implements IYouTubeProvider {
    async isMember(accessToken: string, channelId: string): Promise<boolean> {
        try {
            ...
            const response = await fetch(
                `https://www.googleapis.com/youtube/v3/members?part=snippet&filterByMemberChannelId=${channelId}`,
                {
                    headers: {
