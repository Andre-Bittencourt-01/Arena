...
interface EventsProps {
  ...
  on_navigate_to_result: (event_id: string) => void;
}

const Events: React.FC<EventsProps> = ({ onNavigate, on_navigate_to_result }) => {
  const { events, set_current_event } = useData();
  const [active_tab, set_active_tab] = useState<'upcoming' | 'past'>('upcoming');
  ...
  const filtered_events = events.filter(event => {
    ...
    const status_a = getEventStatus(a);
    const status_b = getEventStatus(b);

    if (active_tab === 'upcoming') {
      if (status_a === 'LIVE' && status_b !== 'LIVE') return -1;
      if (status_b === 'LIVE' && status_a !== 'LIVE') return 1;
    ...
    <div className="mx-auto max-w-5xl px-4 sm:px-6 lg:px-8 py-4 md:py-12 font-sans">
      <div className="flex flex-col gap-4 md:gap-8 mb-6 md:mb-10">
        ...
        <div className="flex items-center gap-2 md:gap-4 border-b border-white/10 overflow-x-auto no-scrollbar pb-0.5">
          <button
            onClick={() => set_active_tab('upcoming')}
            className={`pb-3 px-2 md:pb-4 md:px-4 text-[10px] md:text-sm font-bold uppercase tracking-widest transition-colors border-b-2 shrink-0 ${active_tab === 'upcoming' ? 'text-white border-primary' : 'text-gray-500 border-transparent hover:text-white'}`}
          >
            Próximos
          </button>
          <button
            onClick={() => set_active_tab('past')}
            className={`pb-3 px-2 md:pb-4 md:px-4 text-[10px] md:text-sm font-bold uppercase tracking-widest transition-colors border-b-2 shrink-0 ${active_tab === 'past' ? 'text-white border-primary' : 'text-gray-500 border-transparent hover:text-white'}`}
          >
            Passados
          </button>
        </div>
                ...
                <button
                  onClick={() => {
                    if (getEventStatus(event) === 'COMPLETED') {
                      on_navigate_to_result(event.id);
                    } else {
