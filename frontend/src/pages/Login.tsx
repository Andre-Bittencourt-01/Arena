...
const Login: React.FC<LoginProps> = ({ on_login, on_navigate }) => {
  const { login } = useAuth();
  ...
  const handle_login = async () => {
    ...
    const login_email = email.trim() === '' ? 'andre@arena.com' : email;
    const login_password = password.trim() === '' ? 'a' : password;

    try {
      const success = await login(login_email, login_password);
      if (success) {
        on_login();
      } else {
        set_error('Credenciais inválidas. Tente admin@arenamma.com / admin123');
      }
    } catch (err) {
      set_error('Ocorreu um erro ao tentar fazer login.');
    } finally {
        ...
        <div className="w-full bg-[#180a0a]/80 backdrop-blur-md border border-white/10 rounded-xl shadow-2xl overflow-hidden shrink-0">
          ...
          <div className="p-5 flex flex-col gap-3">
            ...
            <div className="flex items-center justify-between">
              ...
              <button
                onClick={() => on_navigate('forgot-password')}
            ...
            >
              ...
              {!is_loading && <span className="material-symbols-outlined text-base">arrow_forward</span>}
            </button>
