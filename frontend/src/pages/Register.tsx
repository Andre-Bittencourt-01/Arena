...
const Register: React.FC<RegisterProps> = ({ on_navigate, on_register }) => {
    ...
    const validate = () => {
        ...
        if (!form_data.name.trim()) new_errors.name = 'Nome é obrigatório';
        if (!form_data.email.trim()) {
            new_errors.email = 'E-mail é obrigatório';
        } else if (!/\S+@\S+\.\S+/.test(form_data.email)) {
            new_errors.email = 'E-mail inválido';
        }

        if (!form_data.password) {
            new_errors.password = 'Senha é obrigatória';
        } else if (form_data.password.length < 8) {
            new_errors.password = 'Mínimo de 8 caracteres';
        }

        if (form_data.password !== form_data.confirm_password) {
            new_errors.confirm_password = 'As senhas não coincidem';
        }
    ...
    const handle_submit = () => {
        if (validate()) {
            ...
            alert(`Cadastro realizado com sucesso! Bem-vindo, ${form_data.name}.`);
            on_register();
        }
    };
            ...
            <div className="relative z-10 w-full max-w-[480px] p-4 flex flex-col items-center animate-fade-in">
                ...
                <div className="w-full bg-[#180a0a]/90 backdrop-blur-md border border-white/10 rounded-xl shadow-2xl overflow-hidden">
                    <div className="p-8 flex flex-col gap-4">
                        ...
                        {/* Confirm Password */}
                        <div className="flex flex-col gap-1">
                            <label className="text-xs font-bold uppercase tracking-wider text-gray-400 ml-1" htmlFor="confirm_password">Confirmar Senha</label>
                            <input
                                className={`w-full h-12 pl-4 pr-4 bg-black/40 border ${errors.confirm_password ? 'border-red-500' : 'border-white/10'} rounded-lg text-white placeholder-gray-600 focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary transition-all font-normal`}
                                id="confirm_password"
                                placeholder="••••••••"
                                type="password"
                                ...
                                onChange={handle_change}
                            />
                            {errors.confirm_password && <span className="text-red-500 text-[10px] ml-1">{errors.confirm_password}</span>}
                        </div>
