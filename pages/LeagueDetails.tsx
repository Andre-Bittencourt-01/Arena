...
const LeagueDetails: React.FC<LeagueDetailsProps> = ({ leagueId, onNavigate, onBack }) => {
    console.log('LeagueDetails Mounted with ID:', leagueId);
    ...
    const loadLeague = async () => {
        ...
        try {
            ...
            if (leagueData) {
                ...
                const data = await LeagueService.getLeaderboard(leagueData.id);
    ...
    // Top 3 for Podium
    const top3 = leagueLeaderboard.slice(0, 3);
    const first = top3[0];
    const second = top3[1];
    const third = top3[2];
    ...
    const renderUserRow = (user: any, index: number, isHighlight: boolean = false) => {
        ...
        return (
            ...
            >
                {isCurrentUser && !isHighlight && <div className="absolute left-0 top-0 bottom-0 w-0.5 sm:w-1 bg-primary"></div>}

                <div className="col-span-2 sm:col-span-1 flex justify-center">
                    <div className={`font-display text-xl sm:text-3xl font-black italic tracking-tighter ${isHighlight || isCurrentUser ? 'text-primary scale-110' : 'text-white/90 group-hover:text-primary'}`}>
                        {rank}
                    </div>
                </div>

                <div className="flex col-span-2 sm:col-span-3 justify-center sm:border-l border-white/5 sm:h-12 items-center shrink-0">
                    {rankingFilter !== 'week' && renderPerformance(user, 'table')}
                </div>
                ...
                <div className="col-span-6 sm:col-span-5 flex items-center justify-start pl-4 sm:pl-8 sm:border-l border-white/5 sm:h-12 px-2 overflow-hidden">
                    <div className="flex items-center justify-start gap-2 sm:gap-4 w-full">
                        <div className={`size-8 sm:size-10 bg-zinc-800 flex-shrink-0 ${isHighlight || isCurrentUser ? 'ring-1 ring-primary ring-offset-1 ring-offset-zinc-900 shadow-xl shadow-primary/30' : 'border border-white/10'}`} style={{ clipPath: 'polygon(15% 0, 100% 0, 100% 85%, 85% 100%, 0 100%, 0 15%)' }}>
                            <img alt={user.name} className="w-full h-full object-cover" src={user.avatar} />
                        </div>
                        <div className="flex flex-col min-w-0">
                            <div className="flex items-center gap-1.5 min-w-0">
                                <span className={`font-condensed font-bold uppercase tracking-tight text-sm sm:text-2xl truncate ${isHighlight || isCurrentUser ? 'text-primary' : 'text-white/90'}`}>{user.name}</span>
                ...
                <div className="col-span-2 sm:col-span-3 text-center sm:text-right sm:pr-10 sm:border-l border-white/5 sm:h-12 flex flex-col justify-center items-center sm:items-end">
                    <div className={`font-display text-xl sm:text-4xl font-black tracking-tighter leading-none ${isHighlight || isCurrentUser ? 'text-primary' : 'text-white'}`}>
                        {displayPoints}
                    </div>
                </div>
            </div>
    ...
    const currentUserData = leagueLeaderboard.find(u => u.id === currentUser?.id);
    const currentUserIndex = leagueLeaderboard.findIndex(u => u.id === currentUser?.id);
        ...
        <div className="relative z-10 flex-1 max-w-7xl mx-auto w-full px-4 sm:px-6 pt-0 font-display scroll-smooth pb-20 sm:pb-8">
            ...
            <div className="sticky top-0 md:top-0 z-40 bg-background-dark/95 backdrop-blur-md -mx-4 sm:-mx-6 px-4 sm:px-6 pt-0 pb-0 border-b border-white/5">
                ...
                >
                    ...
                    <div className="absolute inset-0 z-30 pointer-events-none flex justify-between items-center px-1">
                        <button disabled={!prevItem} onClick={(e) => { e.stopPropagation(); prevItem && handlePeriodSelect(prevItem.id); }} className={`size-12 flex items-center justify-center rounded-full pointer-events-auto transition-all active:scale-95 ${prevItem ? 'text-white/50 hover:bg-black/40 hover:text-white' : 'text-white/5 opacity-0'}`}>
                            <span className="material-symbols-outlined text-3xl drop-shadow-lg">chevron_left</span>
                        </button>
                        <button disabled={!nextItem} onClick={(e) => { e.stopPropagation(); nextItem && handlePeriodSelect(nextItem.id); }} className={`size-12 flex items-center justify-center rounded-full pointer-events-auto transition-all active:scale-95 ${nextItem ? 'text-white/50 hover:bg-black/40 hover:text-white' : 'text-white/5 opacity-0'}`}>
                            <span className="material-symbols-outlined text-3xl drop-shadow-lg">chevron_right</span>
                        </button>
                    </div>

                    <div className="absolute inset-0 z-20 cursor-pointer active:bg-white/5 transition-colors" onClick={() => setShowFullSelector(true)}>
                        {rankingFilter === 'week' ? (
                            (() => {
                            ...
                            <>
                                ...
                                <div className="absolute inset-0 flex flex-col justify-center items-center px-16 text-center">
                                    ...
                                    <div className="flex flex-col items-center animate-in fade-in zoom-in-95 duration-300 w-full px-2">
                                        <h4 className="font-condensed text-xl sm:text-2xl font-black text-white italic uppercase leading-none drop-shadow-md w-full text-center">{currentItem?.label}</h4>
            ...
            <div className="flex flex-col border border-white/10 bg-zinc-900/40 backdrop-blur-3xl overflow-hidden relative shadow-[0_40px_100px_rgba(0,0,0,0.6)] rounded-2xl">
                ...
                <div className="divide-y divide-white/10 relative z-10 border-t border-white/5">
                    {leagueLeaderboard.length > 3 ? (
                        leagueLeaderboard.slice(3).map((user, index) => renderUserRow(user, index + 3))
                    ) : (
            ...
            {showFullSelector && (
                <div className="fixed top-[60px] md:top-[80px] bottom-[80px] left-3 right-3 z-[40] bg-zinc-950/95 backdrop-blur-md animate-in fade-in duration-200 flex flex-col p-4 sm:p-6 border border-white/10 shadow-2xl rounded-2xl">
                    <div className="flex justify-end mb-2">
                        <button onClick={() => setShowFullSelector(false)} className="size-12 bg-zinc-800 rounded-full flex items-center justify-center text-white hover:bg-zinc-700 shadow-lg border border-white/10 transition-colors"><span className="material-symbols-outlined text-2xl">close</span></button>
                    </div>
                                                ...
                                                if (parts.length === 2) {
                                                    return (
                                                        <div className="flex flex-col gap-0.5 leading-none w-full">
                                                            ...
                                                            <span className={`font-condensed font-black uppercase truncate w-full px-1 text-lg ${item.id === currentItem?.id ? 'text-white' : 'text-white/70'}`}>{parts[0]}</span>
                                                            <span className={`text-[10px] font-bold italic lowercase ${item.id === currentItem?.id ? 'text-white' : 'text-primary'}`}>vs</span>
                                                            <span className={`font-condensed font-black uppercase truncate w-full px-1 text-lg ${item.id === currentItem?.id ? 'text-white' : 'text-white/70'}`}>{parts[1]}</span>
                                                        </div>
                                                    );
                                                }
                                            }
